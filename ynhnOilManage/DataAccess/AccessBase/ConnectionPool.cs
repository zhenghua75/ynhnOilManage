...
namespace ynhnOilManage.DataAccess.Common
{
    /// <summary>
    /// 名称：数据库连接池。
    /// 版本：V1.0
    ...
    public sealed class ConnectionPool : ObjectPool
    {
        ...
        {
			string _connectionStringSet = ynhnOilManage.Common.ConfigAdapter.GetConfigNote("SetConnectionString").Trim();
			if(String.Empty != _connectionStringSet)
			{
				_connectionStringSet = ynhnOilManage.Common.DataSecurity.Encrypt(_connectionStringSet);
				ynhnOilManage.Common.ConfigAdapter.SetConfigNote("ConnectionString",_connectionStringSet);
                ynhnOilManage.Common.ConfigAdapter.SetConfigNote("SetConnectionString", String.Empty);
			}
			
            _connectionString = ynhnOilManage.Common.ConfigAdapter.GetConfigNote("ConnectionString");
			_connectionString = ynhnOilManage.Common.DataSecurity.Decrypt(_connectionString);
        }
        #endregion
